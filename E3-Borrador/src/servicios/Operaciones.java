...
public class Operaciones {
	...
	public Operaciones() {
		...
		this.activoDAO= new ActivoDAOjdbc();
		this.transaccionDAO= new TransaccionDAOjdbc();
	...
	public void cargarUsuario(String nombre, String apellido, String email, String password, Boolean aceptaTerminos) {
		Persona persona = new Persona(nombre, apellido);
		Persona p= personaDAO.cargarPersona(persona);
		Usuario usuario = new Usuario(p.getId(), email, password, aceptaTerminos);
		usuarioDAO.cargarUsuario(usuario);
	}
	
	public boolean verificarUsuario(String email, String password){
			boolean existe=false;
			if(usuarioDAO.usuarioEnBD(email,password)) {
				existe = true;
			}
			return existe;
	}
	...
	//ACTIVOS
	public void cargarActivo(Double cantidad,String nomenclatura,String email){
		if(!monedaDAO.monedaEnBD(nomenclatura)) {
			...
			// Sino se crea
				...
				Activo activo=new Activo(cantidad,nomenclatura,usuarioDAO.obtenerUsuario(email).getId(),monedaDAO.obtenerMoneda(nomenclatura).getId());
	...
	//COMPRA Y SWAP
	public void compra(String cripto, String fiat, Double monto,Scanner scanner,String email) throws SQLException {
		...
		if(!activoDAO.activoEnBD(cripto)) {
			...
			this.cargarActivo(0.0, cripto,email);
		}
	    ...
	    Transaccion transaccion = new Transaccion(resumen, fecha, usuarioDAO.obtenerUsuario(email).getId());
	    transaccionDAO.cargarTransaccion(transaccion);
		System.out.println("Compra realizada con éxito.");
	}
	
	public void swap(String criptoConvertir, Double monto, String criptoEsperada,Scanner scanner, String email) throws SQLException {
		// Se verifica si las criptomonedas son iguales
	    if (criptoConvertir.equalsIgnoreCase(criptoEsperada)) {
	        System.out.println("No se puede hacer un swap entre criptomonedas del mismo tipo.");
	        return;
	    }
		...
		if(!activoDAO.activoEnBD(criptoEsperada)) {
			...
			this.cargarActivo(0.0,criptoEsperada,email);
		}
